      ...
      builder: (context, state) {
        ...
        return ListView.separated(
          itemBuilder: (context, index) {
            //final details = mov[index];

            return MovieDetailsCard(index: index);
          },
          separatorBuilder: (context, index) => const Divider(
            height: 10,
          ),
          itemCount: state.getDetils.length,
          shrinkWrap: true,
        );
      },
    );
...
class MovieDetailsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ComingSoonBloc, ComingSoonState>(
      builder: (context, state) {
        final mov = state.getDetils[index];
        final date = DateTime.parse(mov.data!);
        final formatedDate = DateFormat.yMMMMd('en_US').format(date);
        final da = mov.data!.split('-')[2];
        final mon = formatedDate.split(' ').first.substring(0, 3);
        final kimg = "$kimgAppendUrl${mov.bacdropPath}";
        ...
        // List<int> genreid = mov.genre;

        // print(genreid);

        final gen = getValueAtIndex(mov.genre, 0);

        final g = getGenreName(gen);
        final gen1 = getValueAtIndex(mov.genre, 1);

        final g1 = getGenreName(gen1);
        final gen2 = getValueAtIndex(mov.genre, 2);

        final g2 = getGenreName(gen2);
        // final gen3 = getValueAtIndex(mov.genre, 3);

        // final g3 = getGenreName(gen3);
        // final gen4 = getValueAtIndex(mov.genre, 4);

        // final g4 = getGenreName(gen4);
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        GestureDetector(
                          onTap: () {
                            Share.share('$kimgAppendUrl${mov.postarPath}');
                          },
              ...
              Padding(
                ...
                child: Text(
                  ...
                  style: const TextStyle(
                      color: kLightGreyColor, letterSpacing: 2, fontSize: 15),
                ),
              ),
              const SizedBox(
                height: 10,
